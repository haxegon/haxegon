...
Action Growl is a tiny haxe programming library for beginners.
...
Basic structure:
-------------------------
Action Growl sets everything up in the background, and starts you with a single class, Main.hx.
...
Gfx.createscreen(width, height, scale);
  ...
  - By default, Action Growl has a 768x480 screen with 1:1 scaling. If you want something different,
...
drawimage_scale(x, y, "imagename", scale, pivotx, pivoty);
drawimage_freescale(x, y, "imagename", xscale, yscale, pivotx, pivoty);
drawimage_rotate(x, y, "imagename", rotation, pivotx, pivoty);
drawimage_scale_rotate(x, y, "imagename", scale, rotation, pivotx, pivoty);
drawimage_freescale_rotate(x, y, "imagename", xscale, yscale, rotation, pivotx, pivoty);
drawimage_col(x, y, "imagename", col);
drawimage_scale_col(x, y, "imagename", scale, pivotx, pivoty, col);
drawimage_freescale_col(x, y, "imagename", xscale, yscale, pivotx, pivoty, col);
drawimage_rotate_col(x, y, "imagename", rotation, pivotx, pivoty, col);
drawimage_scale_rotate_col(x, y, "imagename", scale, rotation, pivotx, pivoty, col);
drawimage_freescale_rotate_col(x, y, "imagename", xscale, yscale, rotation, pivotx, pivoty, col);

  - drawimage functions can take the following arguments:
     x - x position on screen
     y - y position on screen
     "imagename" - name of the image you've loaded in (same as the one you pass to addimage())
     scale - float, 1.0 is original size.
     scalex, scaley - freescale functions have different x and y scale values
     col - Hexcode colour in haxe format - e.g. 0xFF0000 draws the image in red.
     rotate - rotation between 0-359
     pivotx, pivoty - position in image to rotate and scale from.
...
Keyboard keys are identified with strings. Here's a full list:
...
Key.justpressed("ENTER");    // True when the key has been pressed this frame

Key.pressed("LEFT");         // True if this key is HELD down

Key.justreleased("SPACE");   // True if the key has been released this frame

Key.delaypressed("Z", 5);    // True if Z is pressed this frame, then false if Z is held 
                             // for five frames. Just a handy shortcut for key delay rates!

Mouse.x                      // Mouse x position.
Mouse.y                      // Mouse y position.

Mouse.justleftpressed()      // True if you've just clicked left this frame
Mouse.justleftreleased()     // True if you've just released left-click this frame

Mouse.justrightpressed()     // True if you've just clicked right this frame
Mouse.justrightreleased()    // True if you've just released right-click this frame
